...
router.afterEach((to, from) => {
  ...
  timeout = setTimeout(async () => {
    if (!catalogueStore.filterLoading && ["catalogue", "catalogue-slug"].includes(to.name as string)) {
      ...
      const url = `${config.public.geonodeBaseUrl}/api/v2/resources?api_preset=catalog_list&filter{metadata_only}=false&page_size=${pageSize.value}${urlQueryParams.value}`

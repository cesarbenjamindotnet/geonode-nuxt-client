...
const getResourceDetailData = async (resourceSelected: any) => {
  ...
  const response = await useFetch<any>(`${config.public.geonodeBaseUrl}/api/v2/${resource_type}/${resourceSelected.pk}?api_preset=viewer_common${presets}`)
...
const addOwnerQueryParam = async (ownerPk) => {
  ...
  const url = `${config.public.geonodeBaseUrl}/api/v2/facets/owner?key=${ownerPk}`
